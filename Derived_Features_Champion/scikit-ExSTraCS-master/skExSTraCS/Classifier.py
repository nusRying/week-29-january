...
class Classifier:
    ...
    def initializeByCovering(self,model,setSize,state,phenotype):
        ...
        # Clamp max specificity to number of attributes to prevent ValueError in random.sample
        max_spec = model.env.formatData.numAttributes
        toSpecify = min(random.randint(1, model.rule_specificity_limit), max_spec)
    ...
    def match(self, model, state):
        for i in range(len(self.condition)):
            ...
            # Discrete
                ...
                if np.array_equal(stateRep, self.condition[i]):
                    pass
                elif stateRep == None:
                    return False
                else:
    ...
    def mutation(self,model,state):
        ...
        # Clamp upper limit to available attributes
        max_limit = min(model.rule_specificity_limit, model.env.formatData.numAttributes)
        
        if (len(self.specifiedAttList) + steps) >= max_limit:
            highLim = max_limit
        else:
